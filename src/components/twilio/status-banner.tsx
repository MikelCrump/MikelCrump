...
export function TwilioStatusBanner({ className }: { className?: string }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setStatus({
        connected: false,
        source: "error",
        message: "Could not reach Twilio status endpoint",
      });
    } finally {
      setLoading(false);
    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-700">
              {status.account?.friendlyName || status.account?.sid}
              {status.phoneNumber ? ` · From ${status.phoneNumber}` : ""}
            </p>
          </div>
        </div>
